...
// 		dd.hwndTarget = m_pMainWnd->ChildWindowFromPoint(pt)->GetSafeHwnd();
 		dd.hwndTarget = WindowFromPoint(pt);
		...
		if (abs(delta.x)>=jog.x || abs(delta.y)>jog.y) {
			...
			BOOL bDrag = (BOOL)m_pMainWnd->SendMessage(WM_DD_DRAGENTER,
				/*::GetDlgCtrlID(m_hwndTracking)*/(WPARAM)m_hwndTracking, (LPARAM)(void*)&dd);
...
BOOL CDragDropMgr::OnLButtonUp(const MSG& msg)
{
	...
	if (IsTargetWnd(dd.hwndTarget)) {
		...
		m_pMainWnd->SendMessage(WM_DD_DRAGDROP, 
			/*pWndTarget->GetDlgCtrlID()*/(WPARAM)dd.hwndTarget, (LPARAM)(void*)&dd);
	} else {
...
// 	CFont* pOldFont = dcMem.SelectObject(pFont);
	CFont font;
	font.Attach((ResFontPool::GetFont(TRUE,FALSE,FALSE,0)));
	...
	CBrush brush;
	brush.CreateSolidBrush(/*GetSysColor(COLOR_HIGHLIGHT)*/RGB(0,0,0));
	dcMem.FillRect(&rc,&brush);
	dcMem.SetBkMode(TRANSPARENT);
	dcMem.SetTextColor(/*GetSysColor(COLOR_WINDOWTEXT)*/ RGB(255, 255, 255));
	OnDrawData(dcMem, CRect(rc.left + 3, rc.top + 2, rc.right, rc.bottom)); // call virtual fn to draw
