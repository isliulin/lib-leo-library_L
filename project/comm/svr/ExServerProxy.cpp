...
void proxy_session::PostCmd(BYTE* pBuf, int nLen)
{
...
BOOL proxy_session::ParseSessionCmd(BYTE* pBuf, int nLen)
{
	// 校验合格后添加到命令队列
	p_t6800_session_->AddCmd(pBuf, nLen, (LPARAM)this);
	return TRUE;
}

int proxy_session::SendCmdParseErrMsg()
{
	m_sendBuf[0] = CF_ERR_CMD;
	m_sendBuf[1] = _ERR_NOT_SPECIFY;
	m_sendBuf[2] = 0;
	return write(m_sendBuf, 3);
}

int proxy_session::ParseGetAlarmMode(BYTE* pBuf, int nLen)
{
	m_sendBuf[0] = CF_ALARM_MODE;
	m_sendBuf[1] = CF_SUB_ALARM_MODE_GET;
	m_sendBuf[2] = p_t6800_session_->m_sys.alarmMode;
	m_sendBuf[3] = p_t6800_session_->m_sys.broadVolumeAdd;
	return write(m_sendBuf, 4);
}

int proxy_session::ParseSetAlarmMode(BYTE* pBuf, int nLen)
{
	...
	p_t6800_session_->m_sys.alarmMode = ByteToAlarmModel(pBuf[2]);
	p_t6800_session_->m_sys.broadVolumeAdd = pBuf[3];
	theDataBase.SetAlarmMode(p_t6800_session_->m_sys.alarmMode, p_t6800_session_->m_sys.broadVolumeAdd);
	p_t6800_session_->m_bNeedSetTime = true;
...
int proxy_session::ParseGetLineInfo(BYTE* pBuf, int nLen)
{
	m_sendBuf[0] = CF_LINE_INFO;
	m_sendBuf[1] = CF_SUB_LINE_INFO_GET;
	int nStructLen = sizeof(SAudioInLineConfig);
	memcpy(&m_sendBuf[2], &p_t6800_session_->m_sys.m_audioInLine.config, nStructLen);
	return write(m_sendBuf, 2+nStructLen);
}

int proxy_session::ParseSetLineInfo(BYTE* pBuf, int nLen)
{
	int nStructLen = sizeof(SAudioInLineConfig);
	...
	memcpy(&p_t6800_session_->m_sys.m_audioInLine.config, &pBuf[2], nStructLen);
...
int proxy_session::ParseGetZoneName(BYTE* pBuf, int nLen)
{
	int ncount = _MAX_ZONEA/16 + (_MAX_ZONEA%16);
	for (int i=0; i<_MAX_ZONEA; i++)
	{
		m_sendBuf[0] = CF_ZONE_INFO;
		m_sendBuf[1] = CF_SUB_ZONE_NAME_GET;
		int i_end = (i+16) < _MAX_ZONEA ? (i+16) : (_MAX_ZONEA-1);
		int nRet = __hton_ZoneName(&m_sendBuf[2], 1000, p_t6800_session_->m_sys.m_zone.it, i, i_end);
		i = i_end + 1;
		if (nRet)
		{
			write(m_sendBuf, 2+nRet);
		}
	}
	return 1;
}

int proxy_session::ParseSetZoneName(BYTE* pBuf, int nLen)
{
	if (__ParseZoneName(pBuf, nLen, p_t6800_session_->m_sys.m_zone) > 0)
	{
...
inline int proxy_session::ParseUserLogin(BYTE* pBuf, int nLen)
{
	S_DB_USER user;
	int nPos = 2;
...
inline int proxy_session::ParseUserLogout(BYTE* pBuf, int nLen)
{
	S_DB_USER user;
	int nPos = 2;
	nPos += ParseByteToUser(user, &pBuf[nPos]);
...
inline int proxy_session::ParseUserAdd(BYTE* pBuf, int nLen)
{
	S_DB_USER user;
	int nPos = 2;
	nPos += ParseByteToUser(user, &pBuf[nPos]);
...
inline int proxy_session::ParseUserDel(BYTE* pBuf, int nLen)
{
	S_DB_USER user;
	int nPos = 2;
	nPos += ParseByteToUser(user, &pBuf[nPos]);
...
inline int proxy_session::ParseUserGetUser(BYTE* pBuf, int nLen)
{
	m_sendBuf[0] = CF_USER;
...
inline int proxy_session::ParseUserGetUserCount(BYTE* pBuf, int nLen)
{
	m_sendBuf[0] = CF_USER;
...
inline int proxy_session::ParseUserGetUsers(BYTE* pBuf, int nLen)
{
	if (nLen != 4)
	{
		return SendCmdParseErrMsg();
		...
		ix++)
	{
		...
		write(m_sendBuf, nPos);
		//Sleep(10);
	}
	return pBuf[4] - pBuf[3];
}

inline int proxy_session::ParseLinkIPGet(BYTE* pBuf, int nLen)
{
	int nPos = 0;
	m_sendBuf[nPos++] = CF_LINK_IP;
	m_sendBuf[nPos++] = CF_SUB_LINK_IP_GET;
	m_sendBuf[nPos++] = p_t6800_session_->m_saTarget.sin_addr.S_un.S_un_b.s_b1;
	m_sendBuf[nPos++] = p_t6800_session_->m_saTarget.sin_addr.S_un.S_un_b.s_b2;
	m_sendBuf[nPos++] = p_t6800_session_->m_saTarget.sin_addr.S_un.S_un_b.s_b3;
	m_sendBuf[nPos++] = p_t6800_session_->m_saTarget.sin_addr.S_un.S_un_b.s_b4;
	m_sendBuf[nPos++] = (char)p_t6800_session_->m_saTarget.sin_port;
	m_sendBuf[nPos++] = (char)(p_t6800_session_->m_saTarget.sin_port>>8);
	return write(m_sendBuf, nPos);
}

inline int proxy_session::ParseLinkIPSet(BYTE* pBuf, int nLen)
{
	if (8!=nLen)
	{
		return SendCmdParseErrMsg();
...
// 	port = (port<<8) | pBuf[nPos];
	port = p_t6800_session_->m_saTarget.sin_port; // 端口设置不开放
	p_t6800_session_->SetTargetSockAddr(sin_addr.S_un.S_addr, port);
	// 保存
...
// 获取项目(不包含详细任务)
int proxy_session::ParseGetGroup(BYTE*pBuf, int nLen)
{
	int nPos = 0;
...
// 添加项目
int proxy_session::ParseGroupAdd(BYTE*pBuf, int nLen)
{
...
// 删除某项目
int proxy_session::ParseGroupDel(BYTE*pBuf, int nLen)
{
	int nPos = 0;
	m_sendBuf[nPos++] = CF_TASK;
...
// 修改某项目名称
int proxy_session::ParseGroupEditName(BYTE*pBuf, int nLen)
{
	if (strlen((char*)&pBuf[3]) <= 0)
	{
		return SendCmdParseErrMsg();
...
int proxy_session::RetGroupTask(_DbTaskGroupVec::iterator& iter)
{
	int nPos = 0;
	m_sendBuf[nPos++] = CF_TASK;
...
// 获取某项目下任务
int proxy_session::ParseGetGroupTask(BYTE*pBuf, int nLen)
{
	_DbTaskGroupVec::iterator group_find_iter = FindGroup(theTaskDB.m_groupVec, (char*)&pBuf[3]);
	if (group_find_iter == theTaskDB.m_groupVec.end())
	{
...
// 修改某项目任务
int proxy_session::ParseSetGroupTask(BYTE*pBuf, int nLen)
{
	_DbTaskGroupVec::iterator group_find_iter = FindGroup(theTaskDB.m_groupVec, (char*)&pBuf[3]);
	if (group_find_iter == theTaskDB.m_groupVec.end())
	{
...
int proxy_session::ParseGetProgramConfig(BYTE*pBuf, int nLen) // 获取运行程序配置信息
{
	int nPos = 0;
	m_sendBuf[nPos++] = CF_TASK;
...
int proxy_session::ParseSetProgramConfig(BYTE*pBuf, int nLen) // 设置运行程序配置信息
{
...
int proxy_session::ParseProgramDown(BYTE*pBuf, int nLen)
{
	...
	SimulateSession ss(p_t6800_session_);
...
// 遥控器
int proxy_session::ParseGetRemoteTask(BYTE*pBuf, int nLen)
{
	int nPos = 0;
	m_sendBuf[nPos++] = CF_TASK;
...
int proxy_session::ParseSetRemoteTask(BYTE*pBuf, int nLen)
{
...
int proxy_session::ParseUserEdit( BYTE* pBuf, int nLen )
{
	S_DB_USER user;
	int nPos = 2;
	nPos += ParseByteToUser(user, &pBuf[nPos]);
