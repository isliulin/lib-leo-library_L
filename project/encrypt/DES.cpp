...
//循环左移 
void CDES::MoveLeft(bool *In, int len, int loop) 
{ 
	memcpy(Tmp, In, loop); 
...
//字节转换成位 
void CDES::Byte2Bit(bool *Out, const char *In, int bits) 
{ 
...
//位转换字节 
void CDES::Bit2Byte(char *Out, const bool *In, int bits) 
{ 
	memset(Out, 0, bits >> 3); 
	for(int i = 0; i < bits; ++ i) 
		Out[i >> 3] |= In[i] << (i & 7); 
...
//S 盒置换 
void CDES::funS(bool Out[32], const bool In[48]) 
{ 
...
//F 函数 
void CDES::funF(bool In[32], const bool Ki[48]) 
{ 
	static bool MR[48]; 
...
//生成子密钥 
void CDES::MakeSubKey(PSubKey pSubKey, const char Key[8]) 
{ 
...
//一重DES加/解密 
void CDES::DES_Original(char Out[8], char In[8], const PSubKey pSubKey, bool Type)
{ 
...
//生成密钥 
void CDES::MakeKey(const char* Key, int len) 
{ 
	memset(deskey, 0, 16);
	memcpy(deskey, Key, len>16?16:len);
	MakeSubKey(&SubKey[0], &deskey[0]);
	len>8 ? (MakeSubKey(&SubKey[1], &deskey[8]), true) : false;
}

bool CDES::DES_EDE2(char *Out, char *In, long datalen, const char *Key, int keylen, bool Type) 
{
	...
	{
		DES_Original(Out, In,  &SubKey[0], Type); 
		DES_Original(Out, Out, &SubKey[1], !Type); 
		DES_Original(Out, Out, &SubKey[0], Type); 
	}
	return true; 
} 
