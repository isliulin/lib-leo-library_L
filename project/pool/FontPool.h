...
#define KUIF_GetFont(key)        ResFontPool::GetFont(key)
...
protected:
	...
	static ResFontPool* ms_pInstance;

	static ResFontPool* _Instance()
	{
		if (!ms_pInstance)
			ms_pInstance = new ResFontPool;
		return ms_pInstance;
	}
...
// __declspec(selectany) CString ResFontPool::ms_strFaceName = _T("Tahoma");
// __declspec(selectany) LONG ResFontPool::ms_lFontSize = -11;

__declspec(selectany) ResFontPool* ResFontPool::ms_pInstance = NULL;
